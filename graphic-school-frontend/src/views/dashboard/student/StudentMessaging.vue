  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">التواصل مع المدربين</p>
      </div>
      <button
            ...
            <div class="flex items-start justify-between gap-2">
              <div class="flex-1 min-w-0">
                <p class="font-semibold text-slate-900 dark:text-white truncate">
                  {{ conversation.instructor?.name || 'مدرب' }}
                </p>
            ...
            <div class="flex items-center justify-between">
              <div>
                <h3 class="font-semibold text-slate-900 dark:text-white">
                  {{ selectedConversation.instructor?.name || 'مدرب' }}
                </h3>
    ...
    <div v-if="showNewConversationModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-2xl w-full">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700">
          <h3 class="text-xl font-bold text-slate-900 dark:text-white">محادثة جديدة</h3>
        </div>
        <form @submit.prevent="createConversation" class="p-6 space-y-4">
          <div>
            <label class="label">الكورس</label>
            <select v-model="newConversationForm.course_id" class="input" required @change="loadInstructors">
          ...
          <div>
            ...
            <select v-model="newConversationForm.instructor_id" class="input" required :disabled="!newConversationForm.course_id">
              <option value="">اختر المدرب</option>
              <option v-for="instructor in courseInstructors" :key="instructor.id" :value="instructor.id">
                {{ instructor.name }}
              </option>
            </select>
          </div>
          <div>
            <label class="label">الموضوع (اختياري)</label>
            <input v-model="newConversationForm.subject" type="text" class="input" />
          </div>
          <div class="flex gap-3 pt-4">
            <button type="submit" class="btn-primary flex-1">بدء المحادثة</button>
            <button type="button" @click="showNewConversationModal = false" class="btn-secondary flex-1">إلغاء</button>
          </div>
        </form>
      </div>
    </div>
...
async function loadMyCourses() {
  try {
    const data = await get('/student/courses');
    myCourses.value = Array.isArray(data) ? data : (data.data || []);
  } catch (err) {
...
async function loadInstructors() {
  if (!newConversationForm.course_id) {
    courseInstructors.value = [];
    return;
  }
  try {
    const course = myCourses.value.find(c => c.id === parseInt(newConversationForm.course_id));
    if (course?.instructors) {
      courseInstructors.value = course.instructors;
    }
  } catch (err) {
...
async function createConversation() {
  try {
    const data = await post('/messaging/conversations', newConversationForm);
    const conversation = Array.isArray(data) ? data[0] : data;
    showNewConversationModal.value = false;
    Object.assign(newConversationForm, {
      course_id: '',
      instructor_id: '',
      subject: '',
    });
