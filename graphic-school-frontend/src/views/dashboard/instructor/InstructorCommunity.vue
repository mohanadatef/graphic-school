<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-2xl font-bold text-slate-900">{{ $t('instructor.community') || 'Community' }}</h2>
        <p class="text-sm text-slate-500">{{ $t('instructor.communityDescription') || 'Engage with students in community discussions' }}</p>
      </div>
      <button
        @click="showCreateModal = true"
        class="px-4 py-2 bg-primary text-white rounded-md inline-block"
      >
        {{ $t('instructor.communityCreatePost') || 'Create Post' }}
      </button>
    </div>
    ...
    <div v-else-if="store.posts.length === 0" class="text-center py-20">
      <p class="text-slate-500 text-lg">{{ $t('instructor.communityNoPosts') || 'No posts yet' }}</p>
    </div>
    ...
    >
      <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h3 class="text-lg font-bold mb-4">{{ editingPost ? 'Edit Post' : 'Create Post' }}</h3>
        <PostForm
          :post="editingPost"
          :groups="myGroups"
