describe('Student E2E Tests', () => {
  ...
  it('1. Student login', () => {
    cy.loginAsStudent();
    // Login command already checks redirect, just verify page loaded
    cy.waitUntilFrontendReady();
    cy.get('body', { timeout: 10000 }).should('be.visible');
    cy.screenshot('student-homepage');
  });

  it('2. Student → Program listing', () => {
    cy.loginAsStudent();
    cy.waitUntilFrontendReady();
    // Navigate to student programs dashboard
    cy.visit('/dashboard/student/programs');
    cy.wait(2000);
    ...
    cy.get('body').should(($body) => {
      ...
      expect(text.includes('Program') || text.includes('program') || text.includes('برنامج')).to.be.true;
    });
  });
  ...
  it('4. Student → Session view', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/sessions');
    cy.wait(2000);
  ...
  it('5. Student → Submit assignment', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/assignments');
    cy.wait(2000);
  ...
  it('6. Student → View gradebook', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/gradebook');
    cy.wait(2000);
  ...
  it('7. Student → View certificates', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/certificates');
    cy.wait(2000);
  ...
  it('8. Student → Community → Create post + like + reply', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/community');
    cy.wait(2000);
    ...
    cy.get('body').then(($body) => {
      ...
      if (createPostBtn.length > 0) {
        ...
        // Like a post
        cy.get('body').then(($body) => {
          const likeBtn = $body.find('button[aria-label*="like"], button[aria-label*="Like"], [data-cy="like-btn"], .like-button').first();
          if (likeBtn.length > 0) {
        ...
        // Reply to a post
        cy.get('body').then(($body) => {
          const replyBtn = $body.find('button[aria-label*="reply"], button[aria-label*="Reply"], [data-cy="reply-btn"]').first();
  ...
  it('9. Student → Gamification → Check XP + leaderboard', () => {
    cy.loginAsStudent();
    cy.visit('/dashboard/student/gamification');
    cy.wait(2000);
    ...
    cy.get('body').then(($body) => {
      ...
      if (leaderboardLink.length > 0) {
        ...
        cy.visit('/dashboard/student/leaderboard');
        cy.wait(2000);
        cy.screenshot('student-leaderboard');
      }
    });
